...
use serde::{de::{DeserializeOwned, Visitor}, Deserialize, Deserializer, Serialize};
use services::{connection_monitoring::NotifyConnectionMonitoring, estimated_table::NotifyEstimatedTimetable, facility_monitoring::NotifyFacilityMonitoring, general_message::NotifyGeneralMessage, production_timetable::NotifyProductionTimetable, situation_exchange::NotifySituationExchange, stop_monitoring::NotifyStopMonitoring, vehicle_monitoring::NotifyVechicleMonitoring};
use std::{fmt, fs};
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        "NotifyProductionTimetable" => {
                            service_type = Some(SiriServiceType::ProductionTimetable(
                                map.next_value()?,
                            ));
                        }
                        "NotifyEstimatedTimetable" => {
                            service_type = Some(SiriServiceType::EstimatedTimetable(
                                map.next_value()?,
                            ));
                        }
                        "NotifyStopMonitoring" => {
                            service_type = Some(SiriServiceType::StopMonitoring(
                                map.next_value()?,
                            ));
                        }
                        "NotifyVehicleMonitoring" => {
                            service_type = Some(SiriServiceType::VehicleMonitoring(
                                map.next_value()?,
                            ));
                        }
                        "NotifyConnectionMonitoring" => {
                            service_type = Some(SiriServiceType::ConnectionMonitoring(
                                map.next_value()?,
                            ));
                        }
                        "NotifyGeneralMessage" => {
                            service_type = Some(SiriServiceType::GeneralMessage(
                                map.next_value()?,
                            ));
                        }
                        "NotifyFacilityMonitoring" => {
                            service_type = Some(SiriServiceType::FacilityMonitoring(
                                map.next_value()?,
                            ));
                        }
                        "NotifySituationExchange" => {
                            service_type = Some(SiriServiceType::SituationExchange(
                                map.next_value()?,
                            ));
                        }
...
impl SIRI {
    ...
    ///
    pub fn from_file<T: DeserializeOwned>(file_path: &str) -> Result<T, Box<dyn std::error::Error>> {
    ...
    /// * `Result<T, Box<dyn std::error::Error>>`
    ///
    pub fn from_str<'a,T: Deserialize<'a>>(xml_str: &'a str) -> Result<T, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_siri_sm_from_file() {
        let result = SIRI::from_file::<Envelope>("src/fixtures/siri_sm/siri-destineo-sm-cus-2-2024-04-04-13-02-25.xml");
        assert!(result.is_ok());
    }
}