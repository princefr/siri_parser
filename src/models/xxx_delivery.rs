                ...
                while let Some(key) = map.next_key::<String>()? {
                    match key.as_str() {
                        ...
                        "ResponseTimestamp" => {
                            ...
                            response_timestamp = value.get("$text").and_then(Value::as_str).map(String::from);
                        }
                        "RequestMessageRef" => {
                            let value: Value = map.next_value()?;
                            request_message_ref = value.get("$text").and_then(Value::as_str).map(String::from);
                        }
                        "SubscriberRef" => {
                            let value: Value = map.next_value()?;
                            subscriber_ref = value.get("$text").and_then(Value::as_str).map(String::from);
                        }
                        "SubscriptionRef" => {
                            let value: Value = map.next_value()?;
                            subscription_ref = value.get("$text").and_then(Value::as_str).map(String::from);
                        }
                        "Status" => {
                            let value: Value = map.next_value()?;
                        ...
                        "ValidUntil" => {
                            let value: Value = map.next_value()?;
                            valid_until = value.get("$text").and_then(Value::as_str).map(String::from);
                        }
                        "ShortestPossibleCycle" => {
                            let value: Value = map.next_value()?;
                            shortest_possible_cycle = value.get("$text").and_then(Value::as_u64).map(|x| x as u32);
                        }
                        ...
                        _ => {
                            return Err(serde::de::Error::unknown_field(
                                ...
                                &["ResponseTimestamp", "RequestMessageRef", "SubscriberRef", "Status", "ValidUntil", "ShortestPossibleCycle"],
                            ));
